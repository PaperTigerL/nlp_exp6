...
evaluator = pytrec_eval.RelevanceEvaluator(relevant_docs, {'ndcg', 'map', 'recip_rank'})
scores = evaluator.evaluate(run)

print("Queries:", len(relevant_qid))
print("NDCG: {:.2f}".format(np.mean([ele["ndcg"] for ele in scores.values()])*100))
print("MAP: {:.2f}".format(np.mean([ele["map"] for ele in scores.values()])*100))
print("MRR: {:.2f}".format(np.mean([ele["recip_rank"] for ele in scores.values()])*100))
